...
export const computeAnimalStatus = (
  ...
  const activeEnrollment = enrollments.find(e => e.animalId === animal.id && e.status === 'Active');
  if (activeEnrollment) {
    ...
    if (activeProtocol) {
      ...
      if (template) {
        template.steps.forEach((step, idx) => {
          ...
          if (daysUntil <= 3) {
            const key = `${activeProtocol.templateId}-step-${idx}-${stepDate}`;
            ...
            protocolAlertMap[key].animals.push(animal.tag);
          }
        });
      }
    }
